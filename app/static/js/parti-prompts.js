window.PARTI_PROMPTS = [
    "bond",
    "element",
    "molecule",
    "life",
    "protein",
    "yin-yang",
    "wood",
    "metal",
    "space",
    "air",
    "fire",
    "water",
    "earth",
    "force",
    "motion",
    "inertia",
    "energy",
    "black hole",
    "gravity",
    "peace",
    "fairness",
    "gender",
    "intelligence",
    "bias",
    "hate",
    "anger",
    "emotion",
    "feeling",
    "love",
    "artificial intelligence",
    "meaning of life",
    "42",
    "0",
    "infinity",
    "imaginary numbers",
    "Fibonacci number",
    "golden ratio",
    "an F1",
    "parallel lines",
    "concentric circles",
    "concurrent lines",
    "congruent triangles",
    "a hot air balloon",
    "The Starry Night",
    "300",
    "101",
    "U.S. 101",
    "commonsense",
    "happiness",
    "hope",
    "insight",
    "inspiration",
    "derision",
    "Salvador Dalí",
    "a shiba inu",
    "a handpalm",
    "an espresso machine",
    "a propaganda poster",
    "The Oriental Pearl",
    "Ha Long Bay",
    "A Vietnam map",
    "A bowl of Pho",
    "a snail",
    "brain coral",
    "a walnut",
    "a capybara",
    "a baby penguin",
    "a cup of boba",
    "a photo of san francisco's golden gate bridge",
    "A picture of some food in the plate",
    "a chair",
    "the Empire State Building",
    "the Sydney Opera House",
    "a hedgehog",
    "a corgi",
    "a robot",
    "robots",
    "a fall landscape",
    "a sunset",
    "a boat",
    "a fox",
    "a red cube",
    "a panda",
    "a space elevator",
    "a city",
    "a fog",
    "a clock",
    "a phone",
    "food",
    "a store front",
    "an armchair",
    "a teapot",
    "an illustration of a teapot",
    "a tiger",
    "a bench",
    "an orange",
    "a laptop",
    "an owl",
    "a train",
    "a cow",
    "a submarine",
    "a whale",
    "a t-shirt",
    "a bowl",
    "a flag",
    "a cat",
    "a towel",
    "a wall",
    "a car",
    "a giraffe",
    "an eagle",
    "a kangaroo",
    "a canal",
    "the grand canyon",
    "lily pads",
    "a street",
    "a house",
    "a fish",
    "a city intersection",
    "a circle",
    "a red circle",
    "a box",
    "a yellow box",
    "a red box",
    "a sphere",
    "a red sphere",
    "a large blue box",
    "a blue metallic sphere",
    "a horse",
    "a pumpkin",
    "a sword",
    "a statue",
    "a logo",
    "a circular logo",
    "a coffee mug",
    "a pig",
    "a squirrel",
    "a hammer",
    "a screwdriver",
    "a handsaw",
    "a power drill",
    "a cocktail",
    "a margarita",
    "an avocado",
    "a kitchen",
    "an iPhone case",
    "a coffee maker",
    "a banana",
    "a violin",
    "a room",
    "a mountain",
    "a bird",
    "a TV",
    "a Christmas tree",
    "The Statue of Liberty",
    "the Eiffel Tower",
    "a zebra",
    "the city of London",
    "a koi fish",
    "a pineapple",
    "a toaster",
    "a sign",
    "a red lego block",
    "teacup",
    "chair",
    "waterfall",
    "a pirate ship",
    "a dragon",
    "a present",
    "a bottle",
    "a book cover",
    "a sweatshirt",
    "matching socks",
    "cash",
    "a wood cabin",
    "a clock tower",
    "a chimpanzee",
    "a hat",
    "sneakers",
    "a roast turkey",
    "a turkey",
    "a plate",
    "a ladder",
    "a Tyrannosaurus Rex",
    "a Stegasaurus",
    "a Triceratops",
    "a Styracosaurus",
    "a Diplodocus",
    "a yellow sticky note",
    "a ball",
    "red balls",
    "an ostrich",
    "a stone path",
    "a wooden deck",
    "an F1 race car",
    "a taxi",
    "a road",
    "a volcano",
    "a large open book",
    "a living room",
    "an elephant",
    "a tree",
    "a dolphin",
    "a rowboat",
    "a crown",
    "a pick-up truck",
    "a key",
    "a goat",
    "a chest",
    "a coffee table",
    "toy cars",
    "a bookshelf",
    "the moon",
    "the Earth",
    "the International Space Station",
    "the planet Jupiter",
    "a tornado",
    "a tidal wave",
    "a laptop screen",
    "an airplane",
    "a butterfly",
    "a lizard",
    "a turtle",
    "an octopus",
    "a book",
    "a moose",
    "a kachina doll",
    "a doorknocker",
    "a rabbit",
    "orange juice",
    "a green pepper",
    "beer",
    "Times Square",
    "the Great Wall",
    "the Kremlin",
    "a pickup truck",
    "a shoe",
    "a plant",
    "a flower",
    "a pair of headphones",
    "a chemtrail",
    "a tennis court",
    "a ceiling fan",
    "a fire hydrant",
    "a wooden post",
    "a trash bin",
    "a piano",
    "a musical note",
    "a harp",
    "a tuba",
    "The Alamo",
    "a windmill",
    "the geyser Old Faithful",
    "a motorcycle",
    "a painting of black and white",
    "the Great Pyramid",
    "the Parthenon",
    "the Millennium Wheel",
    "a marina",
    "a team",
    "a child",
    "a man",
    "a woman",
    "a girl",
    "a person",
    "a boy",
    "a boy and a tiger",
    "a family",
    "a father and a son",
    "a portrait of an old man",
    "a portrait of young girl",
    "a scientist",
    "A dignified beaver wearing glasses, a vest, and colorful neck tie. He stands next to a tall stack of books in a library.",
    "A solitary figure shrouded in mists peers up from the cobble stone street at the imposing and dark gothic buildings surrounding it. an old-fashioned lamp shines nearby. oil painting.",
    "A punk rock squirrel in a studded leather jacket shouting into a microphone while standing on a stump and holding a beer on dark stage.",
    "Horses pulling a carriage on the moon's surface, with the Statue of Liberty and Great Pyramid in the background. The Planet Earth can be seen in the sky.",
    "A robot painted as graffiti on a brick wall. The words 'Fly an airplane' are written on the wall. A sidewalk is in front of the wall, and grass is growing out of cracks in the concrete.",
    "A warrior wombat holding a sword and shield in a fighting stance. The wombat stands in front of the Arc de Triomphe on a day shrouded mist with the sun high in the sky.",
    "A set of 2x2 emoji icons with happy, angry, surprised and sobbing faces. The emoji icons look like dogs. All of the dogs are wearing blue turtlenecks.",
    "A set of 2x2 emoji icons with happy, angry, surprised and sobbing faces. The emoji icons look like macaroons. All of the macaroons are wearing cowboy hats.",
    "A set of 2x2 emoji icons with happy, angry, surprised and sobbing faces. The emoji icons look like colorful macarons. All of the macarons are wearing cowboy hats.",
    "A set of 2x2 emoji icons with happy, angry, surprised and sobbing faces. The emoji icons look like pandas. All of the pandas are wearing colorful sunglasses.",
    "A set of 2x2 emoji icons with happy, angry, surprised and sobbing faces. The emoji icons look like pigs. All of the pigs are wearing crowns.",
    "A richly textured oil painting of a young badger delicately sniffing a yellow rose next to a tree trunk. A small waterfall can be seen in the background.",
    "A portrait of a metal statue of a pharaoh wearing steampunk glasses and a leather jacket over a white t-shirt that has a drawing of a space shuttle on it.",
    "A photograph of the inside of a subway train. There are frogs sitting on the seats. One of them is reading a newspaper. The window shows the river in the background.",
    "A photograph of the inside of a subway train. There are lobsters sitting on the seats. One of them is reading a newspaper. The window shows the ocean in the background.",
    "A photograph of the inside of a subway train. There are raccoons sitting on the seats. One of them is reading a newspaper. The window shows the city in the background.",
    "A photograph of the inside of a subway train. There are red pandas sitting on the seats. One of them is reading a newspaper. The window shows the jungle in the background.",
    "A raccoon wearing formal clothes, wearing a tophat and holding a cane. The raccoon is holding a garbage bag. Oil painting in the style of abstract cubism.",
    "A raccoon wearing formal clothes, wearing a tophat and holding a cane. The raccoon is holding a garbage bag. Oil painting in the style of Egyptian tomp hieroglyphics.",
    "A raccoon wearing formal clothes, wearing a tophat and holding a cane. The raccoon is holding a garbage bag. Oil painting in the style of Hokusai.",
    "A raccoon wearing formal clothes, wearing a tophat and holding a cane. The raccoon is holding a garbage bag. Oil painting in the style of Madhubani art.",
    "A raccoon wearing formal clothes, wearing a tophat and holding a cane. The raccoon is holding a garbage bag. Oil painting in the style of pixel art.",
    "A raccoon wearing formal clothes, wearing a tophat and holding a cane. The raccoon is holding a garbage bag. Oil painting in the style of pointilism.",
    "A raccoon wearing formal clothes, wearing a tophat and holding a cane. The raccoon is holding a garbage bag. Oil painting in the style of Rembrandt.",
    "A raccoon wearing formal clothes, wearing a tophat and holding a cane. The raccoon is holding a garbage bag. Oil painting in the style of traditional Chinese painting.",
    "A raccoon wearing formal clothes, wearing a tophat and holding a cane. The raccoon is holding a garbage bag. Oil painting in the style of Vincent Van Gogh.",
    "A raccoon wearing formal clothes, wearing a top hat and holding a cane. The raccoon is holding a garbage bag. Oil painting in the style of Vincent Van Gogh.",
    "a statue of Abraham Lincoln wearing an opaque and shiny astronaut's helmet. The statue sits on the moon, with the planet Earth in the sky.",
    "a portrait of a statue of the Egyptian god Anubis wearing aviator goggles, white t-shirt and leather jacket. The city of Los Angeles is in the background.",
    "A smiling sloth wearing a leather jacket, a cowboy hat, a kilt and a bowtie. The sloth is holding a quarterstaff and a big book. A shiny VW van with a cityscape painted on it and parked on grass.",
    "A smiling sloth wearing a leather jacket, a cowboy hat, a kilt and a bowtie. The sloth is holding a quarterstaff and a big book. The sloth stands a few feet in front of a shiny VW van. The van has a cityscape painted on it and parked on grass.",
    "A DSLR photo of a shiny VW van that has a cityscape painted on it. A smiling sloth stands on grass in front of the van and is wearing a leather jacket, a cowboy hat, a kilt and a bowtie. The sloth is holding a quarterstaff and a big book.",
    "a poodle wearing a baseball cap holding a dictionary in hand and writing bonez on a chalkboard",
    "a portrait of a statue of anubis with a crown and wearing a yellow t-shirt that has a space shuttle drawn on it",
    "an oil surrealist painting of a dreamworld on a seashore where clocks and watches appear to be inexplicably limp and melting in the desolate landscape. a table on the left, with a golden watch swarmed by ants. a strange fleshy creature in the center of the painting",
    "a portrait of a statue of the Egyptian god Anubis wearing aviator goggles, white t-shirt and leather jacket. A full moon over the city of Los Angeles is in the background at night.",
    "a portrait of a statue of the Egyptian god Anubis wearing aviator goggles, white t-shirt and leather jacket. the skyline of Los Angeles at night can be seen in the background.",
    "a portrait of a statue of the Egyptian god Anubis wearing aviator goggles, white t-shirt and leather jacket, flying over the city of Mars. ",
    "A photo of the Space Shuttle Endeavor painted yellow and flying over the Earth. The continent of South America is visible.",
    "A close-up photo of a wombat wearing a red backpack and raising both arms in the air. Mount Rushmore is in the background.",
    "A penguin wearing aviator goggles and flying confidently next to a bemused eagle. ",
    "A donkey is playing tug-of-war against an octopus. The donkey holds the rope in its mouth. A cat is jumping over the rope.",
    "A wall in a royal castle. There are two paintings on the wall. The one on the left a detailed oil painting of the royal raccoon king. The one on the right a detailed oil painting of the royal raccoon queen. A cute dog looking at the two paintings, holding a sign saying 'plz conserve'",
    "A single beam of light enter the room from the ceiling. The beam of light is illuminating an easel. On the easel there is a Rembrandt painting of a raccoon",
    "The Millennium Wheel next to the Statue of Liberty. The Sagrada Familia church is also visible.",
    "A train ride in the monsoon rain in Kerala. With a Koala bear wearing a hat looking out of the window. There is a lot of coconut trees out of the window.",
    "A photograph of a portrait of a statue of a pharaoh wearing steampunk glasses, white t-shirt and leather jacket.",
    "An oil painting of two rabbits in the style of American Gothic, wearing the same clothes as in the original.",
    "a blue cow is standing next to a tree with red leaves and yellow fruit. the cow is standing in a field with white flowers. impressionistic painting",
    "A wombat sits in a yellow beach chair, while sipping a martini that is on his laptop keyboard. The wombat is wearing a white panama hat and a floral Hawaiian shirt. Out-of-focus palm trees in the background.",
    'A high-contrast photo of a panda riding a horse. The panda is wearing a wizard hat and is reading a book. The horse is standing on a street against a gray concrete wall. Colorful flowers and the word "PEACE" are painted on the wall. Green grass grows from cracks in the street.',
    "The buildings of downtown Manhattan situated at below Mount Everest. The Great Pyramid is in the foreground.",
    "Anime illustration of the Great Pyramid sitting next to the Parthenon under a blue night sky of roiling energy, exploding yellow stars, and chromatic blue swirls",
    "An anime illustration of Sydney Opera House sitting next to Eiffel tower, under a blue night sky of roiling energy, exploding yellow stars, and radiating swirls of blu",
    "A close-up high-contrast photo of Sydney Opera House sitting next to Eiffel tower, under a blue night sky of roiling energy, exploding yellow stars, and radiating swirls of blue",
    "the Sydney Opera House with the Eiffel tower sitting on the right, and Mount Everest rising above",
    "Greek statue of a man comforting a cat. The cat has a big head. The man looks angry.",
    "a robot painted as graffiti on a brick wall. a sidewalk is in front of the wall, and grass is growing out of cracks in the concrete.",
    "a portrait of a statue of a pharaoh wearing steampunk glasses, white t-shirt and leather jacket. dslr photograph.",
    "a real flamingo reading a large open book. a big stack of books is piled up next to it. dslr photograph.",
    "A photo of a hamburger fighting a hot dog in a boxing ring. The hot dog is tired and up against the ropes.",
    "a blue cow is standing next to a tree with red leaves and yellow fruit. the cow is standing in a field with white flowers. impressionistic painting.",
    "A cozy living room with a painting of a corgi on the wall above a couch and a round coffee table in front of a couch and a vase of flowers on a coffee table",
    "Photograph of a wall along a city street with a watercolor mural of foxes in a jazz band.",
    "A high resolution photo of a donkey in a clown costume giving a lecture at the front of a lecture hall. The blackboard has mathematical equations on it. There are many students in the lecture hall.",
    "A black dog sitting on a wooden chair. A white cat with black ears is standing up with its paws on the chair.",
    "a Saint Bernard standing up with its paws in the air. A young girl is seated on the dog's shoulders.",
    "a photograph of a squirrel holding an arrow above its head and holding a longbow in its left hand",
    "An empty fireplace with a television above it. The TV shows a lion hugging a giraffe.",
    "an invisible man wearing horn-rimmed glasses and a pearl bead necklase while looking at his phone",
    "a blue semi-truck and its trailer jumping over a row of motorcycles. there are metal ramps on either side of the motorcycles. ",
    "a white rabbit in blue jogging clothes doubled over in pain while a turtle wearing a red tank top dashes confidently through the finish line",
    "a hot air balloon with a yin-yang symbol, with the moon visible in the daytime sky",
    "a photograph of a fiddle next to a basketball on a ping pong table",
    "a racoon detective using a microscope while riding in a train",
    "a photograph of an ostrich wearing a fedora and singing soulfully into a microphone",
    "a cream-colored labradoodle wearing glasses and black beret teaching calculus at a blackboard",
    "a basketball to the left of two soccer balls on a gravel driveway",
    "a cat patting a crystal ball with the number 7 written on it in black marker",
    "a cat licking a large felt ball with a drawing of the Eiffel Tower on it",
    "a giraffe wearing a white bathing suit and carefully stepping to the edge of a diving board and preparing to dive",
    "a paranoid android freaking out and jumping into the air because it is surrounded by colorful Easter eggs",
    "a mixed media image with a photograph of a woman with long orange hair over a background that is a sketch of a city skyline",
    "a black and orange yin-yang symbol with tiger's heads instead of circles",
    "a painting of an ornate treasure chest with a broad sword propped up against it, glowing in a dark cave",
    "Renaissance portrayals of the Virgin Mary, seated in a loggia. Behind her is a hazy and seemingly isolated landscape imagined by the artist and painted using sfumato.",
    "Oil-on-canvas painting of a blue night sky with roiling energy. A fuzzy and bright yellow crescent moon shining at the top. Below the exploding yellow stars and radiating swirls of blue, a distant village sits quietly on the right. Connecting earth and sky is a flame-like cypress tree with curling and swaying branches on the left. A church spire rises as a beacon over rolling blue hills",
    "Painting of a panic-stricken creature, simultaneously corpselike and reminiscent of a sperm or fetus, whose contours are echoed in the swirling lines of the blood-red sky",
    "a propaganda poster depicting a cat dressed as french emperor napoleon holding a piece of cheese",
    "a white bird in front of a dinosaur standing by some trees",
    "Snow mountain and tree reflection in the lake",
    "a white robot, a red robot and a black robot standing together",
    "a tree reflected in the hood of a blue car",
    "a tree reflected in the sunroof of a blue car",
    "a monarch butterfly hatching from its chrysalis",
    "a glass of orange juice to the right of a plate with buttered toast on it",
    "a bottle of beer next to an ashtray with a half-smoked cigarrette",
    "a pineapple with one beer to its left and two beers on its right",
    "a pickup truck with a horse on its left and two dogs on its right",
    "a brown trash bin with a green compost bin on its left and a blue recycling bin on its right",
    "a woman with long hair next to a luminescent bird",
    "a tall man stooping down to enter a low red sports car",
    "a black dog jumping up to hug a woman wearing a red sweater",
    "a man and a woman standing in the back up an old pickup truck",
    "a boy sitting on the shoulders of a woman who is wearing an elegant dress",
    "a girl with curly black hair jumping off a boulder",
    "a grandmother reading a book to her grandson and granddaughter",
    "a young woman with glasses reading a thick book at a mahogany desk",
    "a man in a business suit on a ladder that is leaning up against the side of a white house",
    "a man pouring milk into a coffee cup to make a latte with a beatiful design",
    "a woman using a sledgehammer to smash an ice sculpture of a goose",
    "a painting of the mona lisa on a white wall",
    "a man eating a glazed donut and a woman eating a chocolate cake",
    "A teddy bear wearing a motorcycle helmet and cape is standing in front of Loch Awe with Kilchurn Castle behind him",
    "A teddy bear wearing a motorcycle helmet and cape is driving a speed boat near the Golden Gate Bridge",
    "A teddy bear wearing a motorcycle helmet and cape is car surfing on a taxi cab in New York City",
    "A teddy bear wearing a motorcycle helmet and cape is riding a motorcycle in Rio de Janeiro with Dois Irmãos in the background",
    "A punk rock squirrel in a studded leather jacket shouting into a microphone while standing on a stump",
    "A punk rock squirrel in a studded leather jacket shouting into a microphone while standing on a lily pad",
    "A punk rock squirrel in a studded leather jacket shouting into a microphone while standing on a boulder",
    "A punk rock frog in a studded leather jacket shouting into a microphone while standing on a stump",
    "A punk rock frog in a studded leather jacket shouting into a microphone while standing on a lily pad",
    "A punk rock frog in a studded leather jacket shouting into a microphone while standing on a boulder",
    "A punk rock platstumppus in a studded leather jacket shouting into a microphone while standing on a stump",
    "A punk rock platypus in a studded leather jacket shouting into a microphone while standing on a lily pad",
    "A punk rock platypus in a studded leather jacket shouting into a microphone while standing on a boulder",
    "A map of the United States made out sushi. It is on a table next to a glass of red wine.",
    "A blue Porsche 356 parked in front of a yellow brick wall",
    "A helicopter flies over the Arches National Park.",
    "A helicopter flies over the Grand Canyon.",
    "A helicopter flies over Yosemite.",
    "A sunken ship becomes the homeland of fish.",
    " A sloth in a go kart on a race track. The sloth is holding a banana in one hand. There is a banana peel on the track in the background.",
    "a photograph of a bird wearing headphones and speaking into a microphone in a recording studio",
    "A teddybear on a skateboard in Times Square, doing tricks on a cardboard box ramp.",
    "A smiling sloth wearing a bowtie and holding a quarterstaff and a big book. A shiny VW van parked on grass.",
    "A smiling sloth wearing a bowtie and holding a quarterstaff and a big book.",
    "A smiling sloth wearing a leather jacket, a cowboy hat, a kilt and a bowtie. The sloth is holding a quarterstaff and a big book.",
    "A smiling sloth wearing a leather jacket, a cowboy hat and a kilt.",
    "A shiny VW van with a cityscape painted on it and parked on grass.",
    "The Statue of Liberty with the Manhattan skyline in the background.",
    "a cream colored labradoodle next to a white cat with black-tipped ears",
    "The Great Pyramid of Giza situated in front of Mount Everest",
    "A photo of a Ming Dynasty vase on a leather topped table.",
    "A table full of food. There is a plate of chicken rice, a bowl of bak chor mee, and a bowl of laksa.",
    "A map of the United States with a pin on San Francisco",
    "A map of the United States made out of sushi on the table.",
    "a cute wooden owl statue holding a large globe of the Earth above its head",
    "a photograph of the mona lisa drinking coffee as she has her breakfast. her plate has an omelette and croissant.",
    "the mona lisa wearing a cowboy hat and screaming a punk song into a microphone",
    "a young badger delicately sniffing a yellow rose, richly textured oil painting",
    "a photograph of a blue porsche 356 coming around a bend in the road",
    "fairy cottage with smoke coming up chimney and a squirrel looking from the window",
    "purple lego dollhouse with a pool and a swing",
    "black bearded dog with an injured leg wearing a cone",
    "brown white and black white guinea pigs eating parsley handed to them",
    "The Rosetta Stone lying on the ground, covered in snow.",
    "a high-quality photograph of an armadillo playing a bagpipe while standing on one leg",
    "a white robot with a red mohawk painted as graffiti on a red brick wall",
    "a panda bear playing ping pong using a blue paddle against an ostrich using a red paddle",
    "Anubis wearing sunglasses and sitting astride a hog motorcyle",
    "a photograph of sand with a bucket, lots of scattered shells but no sandpipers",
    "a shiba inu wearing a beret and black turtleneck",
    "a handpalm with leaves growing from it",
    "panda mad scientist mixing sparkling chemicals",
    "a corgi’s head depicted as an explosion of a nebula",
    "a dolphin in an astronaut suit on saturn",
    "a teddy bear on a skateboard in times square",
    "A Big Ben clock towering over the city of London",
    "A Vietnam map showing Ha Long Bay",
    "A bowl of Pho served with bean sprouts on top",
    "a capybara sitting in a field",
    "a baby penguin wearing a blue hat, red gloves, green shirt, and yellow pants",
    "a collection of glasses is sitting on a table",
    "A bare kitchen has wood cabinets and white appliances",
    "A black and white landscape photograph of a black tree",
    "an ornate, high-backed mahogany chair with a red cushion",
    "a whale breaching in front of the Sydney Opera House",
    "a white country home with a wrap-around porch",
    "a corgi wearing a red bowtie and a purple party hat",
    "robots meditating in a vipassana retreat",
    "a white robot passing a soccer ball to a red robot",
    "a fall landscape with a small cottage next to a lake",
    "a cat looking out of a window at a squirrel on a fence",
    "a white cat with black ears and markings",
    "a white cat and a tabby cat looking at each other",
    "a cat sitting on a stairway railing",
    "a professional photo of a sunset behind the grand canyon",
    "a large white yacht in a calm bay on a sunny day",
    "a large white yacht tossed about in a stormy sea",
    "a baby daikon radish in a tutu walking a dog",
    "an illustration of a baby daikon radish in a tutu walking a dog",
    "a long wooden bench in front of a brick wall",
    "an ornate metal bench by a nature path",
    "a white plastic bench with a high arched back",
    "a tall horse next to a red car",
    "an owl standing on a wire",
    "an owl standing on a telephone wire",
    "the skyline of New York City",
    "beautiful fireworks in the sky with red, white and blue",
    "a steam locomotive speeding through a desert",
    "a submarine floating past a shark",
    "a whale breaching near a mountain",
    "the United States flag next to the flag of Texas",
    "a giraffe with an owl on its head",
    "an owl on top of an elephant's back",
    "an eagle swooping down to catch a mouse",
    "an owl with its wings spread out swooping over a tree",
    "a giraffe eating the bark of a tree",
    "a car and a truck on the road next to a traffic light",
    "a cityscape at night with a full moon",
    "an owl gripping a squirrel in its talons",
    "a bloody mary cocktail next to a napkin",
    "an old-fashioned cocktail next to a napkin",
    "a long-island ice tea cocktail next to a napkin",
    "a margarita next to a napkin",
    "a room with two chairs and a painting of the Statue of Liberty",
    "a mountain and its reflection in a lake",
    "a bird and its reflection in a fountain",
    "The Statue of Liberty on a cloudy day",
    "The Statue of Liberty surrounded by helicopters",
    "the Eiffel Tower in winter",
    "a sunken ship at the bottom of the ocean",
    "a sunken submarine at the bottom of the ocean",
    "the Eiffel Tower in a desert",
    "Golden Gate bridge on the surface of Mars",
    "the city of London on Mars",
    "the city of London on the moon",
    "a stop sign with a large tree behind it",
    "a stop sign knocked over on a sidewalk",
    "teacups surounding a kettle",
    "a dragon breathing fire",
    "a dragon breathing fire on a castle",
    "a dragon breathing fire onto a knight",
    "a view of the Big Dipper in the night sky",
    "a view of the Orion constellation in the night sky",
    "a teddy bear to the right of a toy car",
    "a toy car in front of a teddy bear",
    "a large present with a red ribbon",
    "a large present with a red ribbon to the left of a Christmas tree",
    "a half empty bottle of red wine",
    "a wine bottle with a lit candle stuck in its spout",
    "a wine bottle with a red ribbon wrapped around it",
    "a kids' book cover with an illustration of white dog driving a red pickup truck",
    "milk pouring into a large glass",
    "milk pouring from a glass into a bowl",
    "matching socks with cute cats on them",
    "cash on a wooden table",
    "a wood cabin with a fire pit in front of it",
    "view of a clock tower on a cloudy day",
    "a chimpanzee wearing a bowtie and playing a piano",
    "a black baseball hat with a flame decal on it",
    "black hi-top sneakers with the Nike swoosh",
    "a roast turkey being taken out of the oven",
    "a bamboo ladder propped up against an oak tree",
    "a Tyrannosaurus Rex roaring in front of a palm tree",
    "a Stegasaurus eating ferns",
    "a Triceratops charging down a hill",
    "a Styracosaurus displaying its horns",
    "a Diplodocus standing in a lake",
    "a tabby cat coming through a door",
    "a cat's tail showing under a couch",
    "a light shining on a giraffe in a street",
    "a barred owl peeking out from dense tree branches",
    "a great gray owl with a mouse in its beak",
    "a snowy owl standing in a grassy field",
    "a stone path leading away from a fountain",
    "a wooden deck overlooking a mountain valley",
    "a mouse sitting next to a computer mouse",
    "a tiny dragon landing on a knight's shield",
    "an F1 race car in a Manhattan street",
    "a chess queen to the right of a chess knight",
    "a white pawn attacking a black bishop",
    "a mountain stream with salmon leaping out of it",
    "a living room with a large Egyptian statue in the corner",
    "a stone bust next to an egg and an eggplant",
    "scraps of paper drifting in the wind",
    "the silhouette of an elephant on the full moon",
    "a tree growing out of the middle of an intersection",
    "a can of Spam on an elegant plate",
    "a dolphin jumping over a rowboat",
    "a pick-up truck rolling over a grassy field",
    "a drop-top sports car coming around a bend in the road",
    "the Taj Mahal at sunrise",
    "a small garden with an apple tree behind it",
    "a old-time car with a large front grille",
    "a blue wall with a large framed watercolor painting of a mountain",
    "a yellow wall with a large framed oil painting of a car",
    "an old-fashioned phone next to a sleek laptop",
    "a motorcycle parked in an ornate bank lobby",
    "a hot air balloon landing in a corn field",
    "a wooden toy horse with a mane made of rope",
    "a beach with a cruise ship passing by",
    "a group of penguins in a snowstorm",
    "a Scottish castle next to a loch",
    "a full moon peeking through clouds at night",
    "a crescent moon viewed between tree branches at night",
    "a full moon rising above a mountain at night",
    "the International Space Station flying in front of the moon",
    "a tornado passing over a corn field",
    "a tidal wave approaching a coastal road",
    "an antique car by a beach",
    "a water tower next to a deserted road",
    "a small airplane flying over rolling hills",
    "an airplane flying into a cloud that looks like monster",
    "a blue airplane taxiing on a runway with the sun behind it",
    "a car with tires that have yellow rims",
    "a turtle upside down and spinning on its shell",
    "an octopus fleeing and squirting black ink",
    "a moose by a mountain stream",
    "a kachina doll with feathers on its head and wearing a white dress and brown boots.",
    "a kachina doll standing in sand",
    "a doorknocker shaped like a lion's head",
    "a silver doorknocker on a mahoghany door",
    "a rabbit with white fur and black-tipped ears",
    "a glass of orange juice next to an empty pitcher",
    "a glass of orange juice with an orange peel stuck on the rim",
    "a bottle of light beer with a lemon slice wedged in the rim",
    "a helicopter hovering over Times Square",
    "a traffic jam at Times Square",
    "a prop plane flying low over the Great Wall",
    "a view of the Kremlin on a sunny day",
    "a view of the Kremlin with snow falling",
    "a palm tree forest in front of the Kremlin",
    "a Ferrari Testarossa in front of the Kremlin",
    "a rusty red pickup truck with white wheel rims",
    "a blue pickup truck with a rhinoceros in its flatbed",
    "an orange pickup truck next to a yellow Porsche 911",
    "a pickup truck kicking up dust on a back road",
    "a pickup truck going up a mountain switchback",
    "a pickup truck at the beach at sunrise",
    "a pickup truck under street lights at night",
    "a pair of shoes on a tennis racquet",
    "a shoe with a sock draped over it",
    "a plant growing on the side of a brick wall",
    "a plant at the bottom of a shallow stream",
    "a cow eating a green leafy plant",
    "a bundle of blue and yellow flowers in a vase",
    "a horse chewing a large blue flower",
    "a yellow diamond-shaped sign with a deer silhouette",
    "a yellow diamond-shaped sign with a turtle silhouette",
    "a yellow diamond-shaped sign with a puma silhouette",
    "a yellow diamond-shaped sign with a wooly mammoth silhouette",
    "a pair of headphones on a pumpkin",
    "a pair of headphones on a guitar",
    "a pair of headphones on a statue of a horse",
    "a pair of headphones dangling from a tree branch",
    "a car's wheel crushing a pair of headphones",
    "a chemtrail passing between two clouds",
    "a long chemtrail trailing an airplane in a blue sky",
    "a tennis court with a basketball hoop in one corner",
    "a tennis court that is very wet from lots of rain",
    "a rocking chair next to the net of a tennis court",
    "a ceiling fan with an ornate light fixture",
    "red and yellow balloons hanging from a ceiling fan",
    "a sidewalk next to a wooden post with a blue '5' painted on top",
    "a harp with a carved eagle figure at the top",
    "a tuba with red flowers protruding from its bell",
    "an elephant using its trunk to blow into a tuba",
    "The Alamo with bright white clouds above it",
    "a flock of geese in front of The Alamo",
    "an old-fashioned windmill surrounded by flowers",
    "a windmill farm next to a country road",
    "a family of bears passing by the geyser Old Faithful",
    "an old red truck parked by the geyser Old Faithful",
    "an inflatable rabbit held up in the air by the geyser Old Faithful",
    "a Harley-Davidson motorcycle with a flame decal",
    "a motorcycle in front of an rhinoceros",
    "a chopper decorated with the Stars and Stripes",
    "a painting of black and white with a red flower in the right corner",
    "a gorilla climbing up the side of the Great Pyramid",
    "a beat-up truck at the base of the Great Pyramid",
    "snow covering the Great Pyramid",
    "the sun setting behind the Parthenon",
    "fireworks above the Parthenon",
    "the Millennium Wheel in a snow storm",
    "a marina with a herd of dolphins playing in it",
    "a volcano exploding next to a marina",
    "a team playing baseball at the beach",
    "a man standing on a street corner",
    "a crowd of people watching fireworks by a park",
    "a crowd of people watching fireworks by a city",
    "a smiling man with wavy brown hair and trimmed beard",
    "a woman with long black hair and dark skin",
    "a woman with long black hair and dark skin in a long white dress",
    "a man with long blonde hair, brown eyes and blue jeans",
    "an elderly woman with straight hair and metal-rimmed glasses",
    "a man wearing sunglasses and business suit",
    "a girl with long curly blonde hair and sunglasses",
    "an old man with a long grey beard and green eyes",
    "a woman with tan skin in blue jeans and yellow shirt",
    "a woman with a dog puppet and a cat puppet",
    "a group of skiers are preparing to walk up a sand dune",
    "a child eating a birthday cake near some palm trees",
    "a man with wild hair looking into a crystal ball",
    "Gandalf saying you shall not pass",
    "a mosquito biting a man",
    "a witch riding a broom",
    "a knight holding a long sword",
    "a man reading a book with a prism on its cover",
    "a cricket team walking on to the pitch",
    "a cricketer standing next to a wicket",
    "an elder politician giving a campaign speech",
    "a young girl playing piano",
    "a girl getting a kite out of a tree",
    "a politician giving a speech at a podium",
    "a politician speaking to a large crowd",
    "a child in the air while jumping on a trampoline",
    "the Beatles crossing Abbey road",
    "a girl riding an ostrich",
    "a river with people swimming in it as a boat goes by",
    "a selfie of an old man with a white beard",
    "a young girl wearing a tiara and frilly dress",
    "a ballet dancer next to a waterfall",
    "a judge delivering a sentence to the defendant",
    "people packed on a double-decker bus",
    "a man riding a camel on the beach",
    "a woman singing into a microphone",
    "a child unraveling a roll of toilet paper",
    "a girl examining an ammonite fossil",
    "a fairy flying over a girl's shoulder",
    "Tibetan priests ringing a bell",
    "a man sleeping in a hammock",
    "a library filled with kids reading books",
    "a scientist accepting an award",
    "A tornado made of sharks crashing into a skyscraper. painting in the style of Hokusai.",
    "A tornado made of sharks crashing into a skyscraper. painting in the style of abstract cubism.",
    "A tornado made of sharks crashing into a skyscraper. painting in the style of watercolor.",
    "A tornado made of tigers crashing into a skyscraper. painting in the style of Hokusai.",
    "A tornado made of tigers crashing into a skyscraper. painting in the style of abstract cubism.",
    "A tornado made of tigers crashing into a skyscraper. painting in the style of watercolor.",
    "A tornado made of bees crashing into a skyscraper. painting in the style of Hokusai.",
    "A tornado made of bees crashing into a skyscraper. painting in the style of abstract cubism.",
    "A tornado made of bees crashing into a skyscraper. painting in the style of watercolor.",
    "A television made of water that displays an image of a cityscape at night.",
    "A photo of a light bulb in outer space traveling the galaxy with a sailing boat inside the light bulb.",
    "A shiny robot wearing a race car suit and black visor stands proudly in front of an F1 race car. The sun is setting on a cityscape in the background. comic book illustration.",
    "A horse sitting on an astronaut's shoulders.",
    "The collision of two black holes in the center of a galaxy.",
    "a super math wizard cat, richly textured oil painting",
    "A group of farm animals (cows, sheep, and pigs) made out of cheese and ham, on a wooden board. There is a dog in the background eyeing the board hungrily.",
    "A giant cobra snake made from corn",
    "A giant cobra snake made from sushi",
    "A giant cobra snake made from pancakes",
    "A giant cobra snake made from salad",
    "A photo of an astronaut riding a horse in the forest. There is a river in front of them with water lilies.",
    "A rhino beetle this size of a tank grapples a real life passenger airplane on the tarmac",
    "a massive robot with a coffee cup head. it is standing in the street with one foot smashing a car.",
    "A large city fountain that has milk instead of water. Several cats are leaning into the fountain.",
    "A bowl of soup that looks like a monster knitted out of wool",
    "A bowl of soup that looks like a monster made out of plasticine",
    "A bowl of soup that looks like a monster spray-painted on a wall",
    "A bowl of soup that looks like a monster with tofu says deep learning",
    'The 1970s logo for a london-area football club called "The Rumbury Wanderers"',
    "a swordfish and a narwhal fencing in an underwater sandy arena. a crap and a lobster are cheering.",
    "A castle made of tortilla chips, in a river made of salsa. There are tiny burritos walking around the castle",
    "A city in 4-dimensional space-time",
    "A high resolution photo of a rat working out in a gym.",
    "A high resolution photo of a chicken working out in a gym.",
    "a dump truck filled with soccer balls scuba diving in a coral reef.",
    "a yellow dump truck filled with soccer balls driving in a coral reef. a blue whale looms in the background.",
    "A photo of llama wearing sunglasses standing on the deck of a spaceship with the Earth in the background.",
    "A high resolution photo of a large bowl of ramen. There are several origami boats in the ramen of different colors.",
    "an elf drinking orange juice through a straw of a giant orange next to a squirrel and an owl watching from above",
    "An alien octopus floats through a portal reading a newspaper. DSLR photo.",
    "A photo of an astronaut riding a horse in the forest.",
    "A photograph of a bird made of wheat bread and an egg.",
    "a horse corral with tigers standing in each stall",
    "a bike rack with some bike locks attached to it but no bicycles",
    "A red dragon dressed in a tuxedo and playing chess. The chess pieces are fashioned after robots.",
    "An oil painting of a two-story house lifting off the ground like a rocket.",
    "a portrait of a postal worker who has forgotten their mailbag",
    "a politician wearing a soccer jersey and holding a volleyball while giving a speech on a stage",
    "a kangaroo hopping in the air between two identical large statues of warrior rabbits",
    "a impressionistic painting of a scholarly badger reading the Rosetta Stone",
    "A black dragon perched on top of a tall Egyptian obelisk and breathing flames at a knight on the ground",
    "a photograph of a knight in shining armor holding a basketball",
    "Dogs playing poker",
    "Dogs sitting around a poker table",
    "Dogs sitting around a poker table with beer bottles and chips. Their hands are holding cards.",
    "Salvador Dalí with a robotic half face",
    "an espresso machine that makes coffee from human souls",
    "panda mad scientist",
    "a snail made of harp",
    "a giraffe made of turtle",
    "a giraffe imitating a turtle",
    "a cube made of porcupine",
    "a watermelon chair",
    "a giant gorilla at the top of the Empire State Building",
    "a spaceship that looks like the Sydney Opera House",
    "a hedgehog using a calculator",
    "a cat playing checkers",
    "a hamster dragon",
    "a futuristic city",
    "corgi pizza",
    "a clock with no hands",
    "a baby daikon radish in a tutu",
    "a chimpanzee sitting on a wooden bench",
    "a smiling banana wearing a bandana",
    "an orange wearing a cowboy hat",
    "a cat drinking a pint of beer",
    "a train going to the moon",
    "a subway train in an empty station",
    "a cow jumping over the moon",
    "a cat sitting in a car seat",
    "a friendly car",
    "two cats doing research",
    "a red train is coming down the beach",
    "a small kitchen with a white goat in it",
    "a giraffe with a funny face",
    "a peaceful lakeside landscape with migrating herd of sauropods",
    "a cat standing on a horse",
    "a horse standing on an elephant",
    "a cat chasing a horse",
    "a horse chasing a cat",
    "a cat with four eyes",
    "a sword slicing through a loaf of bread",
    "a sword slicing through pouring milk",
    "a coffee mug floating in the sky",
    "a pig face with an eye patch",
    "a stork playing a violin",
    "a TV on a horse",
    "a Christmas tree on a toy train",
    "The Statue of Liberty with the face of an owl",
    "a zebra with blue stripes",
    "a zebra with alternating blue and red stripes",
    "a yellow tiger with blue stripes",
    "a tiger wearing a tuxedo",
    "the statue of Liberty next to the Washington Monument",
    "a koi fish flying in the sky",
    "a pineapple surfing on a wave",
    "a toaster shaking hands with a microwave",
    "blue apples on a tree with yellow leaves",
    "square blue apples on a tree with circular yellow leaves",
    "square red apples on a tree with circular green leaves",
    "a pirate ship landing on the moon",
    "a turkey walking in the kitchen",
    "a horned owl with a graduation cap and diploma",
    "a panda bear with aviator glasses on its head",
    "a nerdy bear wearing glasses and a bowtie",
    "a volcano spewing fish into the sky",
    "a tree with leaves that look like purple balloons",
    "a pony with a shooting star on its flank",
    "a half moon in the day sky",
    "the moon with a smiling face",
    "a tornado with a house carried along at its top",
    "a penguin that is a car",
    "a bicycle wheel that is made of red yarn",
    "a rabbit sitting on a turtle's back",
    "a rabbit wearing a black tophat and monocle",
    "a full pitcher of beer with an elephant's trunk in it",
    "an elephant walking on the Great Wall",
    "a spaceship landing on the Great Wall",
    "an ocean at the base of the Great Wall",
    "a herd of buffalo stampeding at the Kremlin",
    "a pickup truck missing its wheels",
    "a flower with a cat's face in the middle",
    "a flower with large red petals growing on the moon's surface",
    "a grand piano next to the net of a tennis court",
    "a piano tumbling down a hill",
    "a drawing of a series of musical notes wrapped around the Earth",
    "a tuba made of flower petals",
    "a spaceship hovering over The Alamo",
    "a motorcycle hanging on a garage wall",
    "a dragon perched on top of the Great Pyramid",
    "a flying boat gliding past the Parthenon",
    "the Parthenon in front of the Great Pyramid",
    "a diplodocus standing in front of the Millennium Wheel",
    "a child and a penguin sitting on the moon",
    "Superman shaking hands with Spiderman",
    "a man with puppet that looks like a king",
    "a person with arms like a tree branch",
    "a man riding a cat",
    "a triangle with a smiling face",
    "a square with an angry face",
    "a concert without any fans",
    "a summer tree without any leaves",
    "a classroom without any students",
    "a bookshelf without any books on it",
    "a shoe rack without any pairs of shoes on it",
    "a closet without clothes",
    "a plate that has no bananas on it. there is a glass without orange juice next to it.",
    "A bird gives an apple to a squirrel",
    "A squirrel gives an apple to a bird",
    "A dog gives an apple to a squirrel",
    "The sculpture rolled off the shelf because it wasn't level",
    "The sculpture rolled off the shelf because it wasn't anchored",
    "The large ball crashed right through the table because it was made of styrofoam",
    "The large ball crashed right through the table because it was made of steel",
    "The trophy doesn't fit into the brown suitcase because it's too small",
    "The trophy doesn't fit into the brown suitcase because it's too large",
    "The coach smiled at the player tossed the frisbee",
    "The rat the cat the dog bit chased escaped.",
    "To a squirrel, a dog gives an apple.",
    "A man gives a woman a laptop and a boy a book.",
    "A robot gives a wombat an orange and a lemur a banana.",
    "A man sips a latte and a woman a beer.",
    "A rabbit checks its watch, and so does a gecko.",
    "A boy holds and a girl paints a piece of wood.",
    "A cat that has black fur bats a red Christmas ornament that has silver sparkles.",
    "The mouse the cat watches is jumping in the air.",
    "Four deer surrounding a moose.",
    "supercalifragilisticexpialidocious",
    "Incomprehensibilities",
    "Pneumonoultramicroscopicsilicovolcanoconiosis",
    "a large stone pedestal without a statue of a horse on it",
    "a brown mouse laughing at a gray cat because a 16 ton weight is about to fall on its head",
    "a black dog sitting between a bush and a pair of green pants standing up with nobody inside them",
    "The horse raced past the barn fell",
    "The old man the boat.",
    "We painted the wall with cracks.",
    "The raft floated down the river sank.",
    "Mary gave the child the dog bit a Band-Aid.",
    "The man who whistles tunes pianos.",
    "One morning I chased an elephant in my pajamas",
    "A tourist is looking at a whale using a binocular",
    "The dog chased the cat, which ran up a tree. It waited at the top.",
    "The dog chased the cat, which ran up a tree. It waited at the bottom.",
    "An aerial view of Ha Long Bay without any boats",
    "a bench without any cats on it",
    "a laptop no letters on its keyboard",
    "a subway train with no cows in it",
    "a kitchen without a refrigerator",
    "several lily pads without frogs",
    "a street without vehicles",
    "a house with no windows",
    "a car with no windows",
    "a fish without eyes",
    "a city intersection without cars",
    "a horse without a rider",
    "a banana without its peel",
    "a bat landing on a baseball bat",
    "a bird landing on a bat",
    "a rowboat without paddles",
    "a harp without any strings",
    "a marina without any boats in it",
    "Zoomed out view of a giraffe and a zebra in the middle of a field covered with colorful flowers",
    "Three-quarters front view of a blue 1977 Corvette coming around a curve in a mountain road and looking over a green valley on a cloudy day.",
    "Three-quarters front view of a blue 1977 Ford F-150 coming around a curve in a mountain road and looking over a green valley on a cloudy day.",
    "Three-quarters front view of a blue 1977 Porsche 911 coming around a curve in a mountain road and looking over a green valley on a cloudy day.",
    "Three-quarters front view of a red 1997 Corvette coming around a curve in a mountain road and looking over a green valley on a cloudy day.",
    "Three-quarters front view of a red 1997 Ford F-150 coming around a curve in a mountain road and looking over a green valley on a cloudy day.",
    "Three-quarters front view of a red 1997 Porsche 911 coming around a curve in a mountain road and looking over a green valley on a cloudy day.",
    "Three-quarters front view of a yellow 2017 Corvette coming around a curve in a mountain road and looking over a green valley on a cloudy day.",
    "Three-quarters front view of a yellow 2017 Ford F-150 coming around a curve in a mountain road and looking over a green valley on a cloudy day.",
    "Three-quarters front view of a yellow 2017 Porsche 911 coming around a curve in a mountain road and looking over a green valley on a cloudy day.",
    'A photo of a frog reading the newspaper named "Toaday" written on it. There is a frog printed on the newspaper too.',
    "The frog found itself in the newspaper",
    "A cat dreaming about becoming a tiger",
    "A close-up of two chameleons wearing karate uniforms and fighting, jumping over a waterfall.",
    "A close-up of two mantis wearing karate uniforms and fighting, jumping over a waterfall.",
    "A close-up of two beetles wearing karate uniforms and fighting, jumping over a waterfall.",
    "A robot with a black visor and the number 42 on its chest. It stands proudly in front of an F1 race car. The sun is setting on a cityscape in the background. wide-angle view. comic book illustration.",
    "A smiling sloth is wearing a leather jacket, a cowboy hat, a kilt and a bowtie. The sloth is holding a quarterstaff and a big book. The sloth is standing on grass a few feet in front of a shiny VW van with flowers painted on it. wide-angle lens from below.",
    "A smiling sloth wearing a leather jacket, bowtie, kilt and cowboy hat. The sloth is holding a quarterstaff and a big book. The sloth is standing on grass a few feet in front of a rusty old VW van with flowers painted on it. wide-angle lens from below.",
    "view of a giraffe and a zebra in the middle of a field",
    "Aerial view of downtown Manhattan, but with Millennium Wheel next to the Statue of Liberty. The Great Pyramid is on a sandy island near the buildings.",
    "Ground view of the Great Pyramids and Sphinx on the moon's surface. The back of an astronaut is in the foreground. The planet Earth looms in the sky.",
    "Saturn rises on the horizon.",
    "Mars rises on the horizon.",
    "Jupiter rises on the horizon.",
    "a close-up of a blue dragonfly on a daffodil",
    "long shards of a broken mirror reflecting the eyes of a great horned owl",
    "side view of a brown horse wearing a saddle. the number 55 is stamped in white on its rear flank.",
    "three quarters view of a rusty old red pickup truck with white doors and a smashed windshield",
    "view from below of a tall white ladder with just one rung leaning up against a yellow brick wall",
    "a photo of the back of a covered wagon. a polar bear is sticking it's head out of the wagon.",
    "a close up of a handpalm with leaves growing from it",
    "a corgi’s head",
    "An aerial view of Ha Long Bay",
    "a macro photograph of brain coral",
    "a cross-section view of a walnut",
    "an extreme close-up view of a capybara sitting in a field",
    "an overhead view of the Empire State Building",
    "a pig face",
    "a close-up of a bloody mary cocktail",
    "a close-up of an old-fashioned cocktail",
    "a close-up of a long-island ice tea cocktail",
    "a close-up of a margarita",
    "the back of a violin",
    "view of a clock tower from above",
    "view of a clock tower from below",
    "an aerial photo of a carnival at night",
    "an aerial photo of a baseball stadium",
    "an aerial photo of a sandy island in the ocean",
    "an ostrich's face",
    "a close-up of an ostrich's face",
    "the eyes of an owl",
    "a close-up of the eyes of an owl",
    "a top down view of a horse running in a field",
    "tall buildings seen through a window with rain on it",
    "trees seen through a car window on a rainy day",
    "a view of the Earth from the moon",
    "an image of the moons of the planet Jupiter",
    "the eye of the planet Jupiter",
    "a close-up of the eye of the planet Jupiter",
    "an aerial view of the Great Wall",
    "view of the Great Wall from its base",
    "an overhead view of a pickup truck with boxes in its flatbed",
    "a close-up of the keys of a piano",
    "an aerial view of the Great Pyramid",
    "a view of the Milllenium Wheel from the Thames",
    "overhead view of three people looking down at the street from the top of a tall building",
    "two people facing each other",
    "two people facing the viewer",
    "a three quarters view of a man getting into a car",
    "a stack of three red cubes with a blue sphere on the right and two green cones on the left",
    "A photo of a Persian Metal Engraving vase sitting to the left of a bunch of orange flowers.",
    "A photo of a Japanese Porcelain Imari vase on the ground below a wooden chair.",
    "a pen-and-ink crosshatched drawing of a sphere with dark square on it",
    "a metallic blue sphere to the left of a yellow box made of felt",
    "a blue wooden pyramid on top of a red plastic box",
    "concentric squares fading from yellow on the outside to deep orange on the inside",
    "A green heart with shadow",
    "a red cube on top of a blue cube",
    "a white flag with a red circle next to a solid blue flag",
    "a red circle on top of a blue square",
    "a yellow box to the right of a blue sphere",
    "a red box to the right of a green sphere",
    "a red box next to a blue box",
    "a red sphere on top of a yellow box",
    "a large blue box with three small yellow boxes on it",
    "several red lego blocks and one blue one",
    "a red block to the left of a blue pyramid",
    "three small yellow boxes on a large blue box",
    "a black background with a large yellow circle",
    "a white background with a large blue circle",
    "a black background with a large yellow square",
    "a white background with a large blue square",
    "a black background with a large yellow circle and a small red square",
    "a white background with a large blue circle and a small green square",
    "a large yellow triangle above a green square and red rectangle",
    "two small circles to the left of a red triangle that is on a green rectangle",
    "a green pyramid in front of a blue box",
    "a large yellow sphere behind a small purple pyramid",
    "a metallic blue sphere to the left of a brown cardboard box",
    "a yellow swirl next to a blue dashed line.",
    "a red swirl above a black dashed line.",
    "a green pepper to the left of a red pepper",
    "a brown trash bin to the left of a blue recycling bin",
    "a circular brown trash bin in front of a brick wall",
    "ten red apples",
    "Four dragons surrounding a dinosaur",
    "Four cats surrounding a dog",
    "two baseballs to the left of three tennis balls",
    "a group of not more than five meerkats standing with the sun setting behind them",
    "a basketball game between a team of four cats and a team of three dogs",
    "the hands of a single person holding a basketball",
    "several people putting their hands on a basketball",
    "a tiny football in front of three yellow tennis balls",
    "two beat-up baseballs on either side of a yellow basketball",
    "two baseballs next to three tennis balls",
    "7 dogs sitting around a poker table",
    "7 dogs sitting around a poker table, two of which are turning away.",
    "300 movie titles",
    "a collection of glasses",
    "two chairs",
    "three chairs",
    "five chairs",
    "two wooden chairs and three metal chairs",
    "three black cats standing next to two orange cats",
    "a bunch of laptops piled on a sofa",
    "four owls standing on a telephone wire",
    "an owl family",
    "a comic about an owl family in the forest",
    "two cats",
    "a comic about two cats doing research",
    "three elephants standing on top of each other",
    "two red boxes",
    "two violins standing up",
    "two violins standing up with their bows on the ground in front of them",
    "three violins lying on the floor",
    "three red lego blocks",
    "four teacups surounding a kettle",
    "three small yellow boxes",
    "bottles",
    "two frosted glass bottles",
    "five frosted glass bottles",
    "three green glass bottles",
    "four green glass bottles",
    "two wine bottles",
    "three wine bottles",
    "four wine bottles",
    "ten wine bottles",
    "two wine bottles and three beer cans",
    "a pair of matching socks with cute cats on them",
    "a pile of cash on a wooden table",
    "a pile of cash on a stone floor",
    "two red balls",
    "two red balls on a table",
    "five red balls",
    "five red balls on a table",
    "three yellow balls and two red boxes on a table",
    "an antique chest with three drawers",
    "ten triangles and five squares on a black background",
    "a pile of toy cars",
    "a full bookshelf with three shelves",
    "a bookshelf with ten books stacked vertically",
    "three airplanes parked in a row at a terminal",
    "three green peppers",
    "five green peppers to the right of two red onions",
    "Times Square with thousands of dogs running around",
    "three pickup trucks piled on top of each other",
    "a pair of brown suede shoes",
    "two red flowers and three white flowers",
    "two parallel chemtrails in blue sky",
    "two chemtrails forming an X in blue sky",
    "a tennis court with three yellow cones on it",
    "a tennis court with tennis balls scattered all over it",
    "a ceiling fan with four white blades",
    "a ceiling fan with five brown blades",
    "two pianos next to each other",
    "a series of musical notes",
    "a field with ten massive modern windmills",
    "two motorcycles facing each other",
    "children",
    "one child on a couch",
    "three children on a couch",
    "ten children on a couch",
    "a crowd of people watching fireworks",
    "a parade with cars and people waving",
    "a family of four posing in front of a house",
    "a family of four posing at the Grand Canyon",
    "a family of four posing at Mount Rushmore",
    "a family of four posing on the moon",
    "a family of four walking at the beach with waves covering their feet",
    "a jungle gym with three kids on it",
    "four people riding in a convertible car",
    "two runners crossing the finish line together",
    "a four-piece band on a stage in front of a small crowd",
    "two sets of identical twins",
    "Siberian husky playing the piano.",
    "A van parked on grass",
    "a smiling sloth",
    "A shiny VW van parked on grass.",
    "a farm scene with cows, ducks and a tractor.",
    "a lavender backpack with a triceratops stuffed animal head on top",
    "a dolphin in an astronaut suit",
    "a teddy bear on a skateboard",
    "A tiger is playing football",
    "A Google map highlighting Vietnam",
    "A bowl of Chicken Pho",
    "A bowl of Beef Pho",
    "a lovestruck cup of boba",
    "a horse in a field of flowers",
    "A green heart",
    "The sunset on the beach is wonderful",
    "A red bus is driving on the road",
    "A picture of a very tall stop sign",
    "A movie poster of mountain and sea",
    "A giraffe walking through a green grass covered field",
    "A green train is coming down the tracks",
    "A living area with a television and a table",
    "A red hydrant on the grass",
    "Sunset over the sea",
    "A small house in the wilderness",
    "Sunset over the skyline of a city",
    "A heart made of chocolate",
    "A heart made of water",
    "A heart made of wood",
    "A heart made of cookie",
    "a white country home",
    "robots meditating",
    "a robot kicking a soccer ball",
    "a cat looking out of a window",
    "a cat coming through a cat door",
    "a cat jumping in the air",
    "a cat jumping in the air to catch a bird",
    "a cat jumping in the air to get onto a table",
    "a sunset behind the grand canyon",
    "a boat in the canals of venice",
    "a large white yacht",
    "a panda eating bamboo",
    "a fog rolling into new york",
    "a green clock",
    "a photo of a phone from the 20s",
    "a photo of the food of china",
    "a baby daikon radish",
    "a very fancy French restaurant",
    "a tiger standing by some flowers",
    "a bench next to a flower pot",
    "a bird standing on a stick",
    "a blue sports car on the road",
    "a red sports car on the road",
    "a soccer ball flying over a car",
    "the finale of a fireworks display",
    "beautiful fireworks in the sky",
    "a subway train",
    "a subway train coming out of a tunnel",
    "orange jello",
    "a yellow t-shirt",
    "a blue t-shirt",
    "a flag furling in the wind",
    "a white towel",
    "a white towel with a cat on it",
    "a black towel",
    "a black towel with a dog on it",
    "a yellow wall",
    "a friendly car in the city",
    "an owl family in the forest",
    "a kangaroo jumping through the park",
    "a street in Paris",
    "a canal in Venice",
    "a small house",
    "a small house on a mountain top",
    "the grand canyon on a cloudy day",
    "a house on a mountain",
    "a photograph of a house on a mountain",
    "a peaceful lakeside landscape",
    "a cat reading a book",
    "a cat reading a newspaper",
    "a cat reading a comic book",
    "a horse reading a book",
    "a horse reading a newspaper",
    "a horse reading a comic book",
    "a cat jumping down from a wall",
    "a pumpkin with a candle in it",
    "a sword in a stone",
    "an Egyptian statue",
    "an Egyptian statue in the desert",
    "a massive statue in a temple",
    "a circular logo on a coffee mug",
    "a pig in a field",
    "a horse in a field",
    "a squirrel in a field",
    "a tiger in a field",
    "a pig in a forest",
    "a horse in a forest",
    "a squirrel in a forest",
    "a tiger in a forest",
    "a hammer on a table",
    "a screwdriver on a table",
    "a handsaw on a table",
    "a power drill on a table",
    "a bloody mary cocktail",
    "an old-fashioned cocktail",
    "a long-island ice tea cocktail",
    "a tree surrounded by flowers",
    "a street with several cars on it",
    "an avocado on a table",
    "a kitchen with a large refrigerator",
    "weeds in the cracks of a sidewalk",
    "a wood treehouse in an oak tree",
    "a metal treehouse in an oak tree",
    "a violin next to an apple",
    "a room with two chairs and a painting",
    "a canal in Amsterdam",
    "red apples on a tree with green leaves",
    "a stop sign",
    "a yield sign",
    "a mountain with a cloud hanging over it",
    "a bottle of red wine",
    "wine bottles",
    "the cover of a book about gardening",
    "a milk container on a table",
    "a milk container in a refrigerator",
    "a marine iguana crossing the street",
    "cash on a stone floor",
    "a half-peeled banana",
    "a black baseball hat",
    "black hi-top sneakers",
    "a roast turkey on the table",
    "a plate with white rice topped by cooked vegetables",
    "slices of avocado on a piece of toast",
    "slices of mango on a piece of toast",
    "a bamboo ladder",
    "a cat jumps over a baby gate",
    "an ostrich standing on a couch",
    "a taxi driving in the countryside",
    "a two lane road with a bright yellow line ",
    "a road ruined by an earthquake",
    "a volcano erupting near a small town",
    "a volcano with lava pouring down its slopes",
    "a horse running in a field",
    "a robot cooking",
    "a robot cooking in the kitchen",
    "a compass next to a piece of fruit",
    "the silhouette of an elephant",
    "a tree growing through a fence",
    "a butterfly kite stuck high in a tree",
    "a unicorn with a multicolored mane",
    "a crown with a ruby in its center",
    "an ornate jewel-encrusted key",
    "a goat wearing headphones",
    "a small tree covered in white blossoms",
    "an antique chest",
    "a coffee table with a magazine on it",
    "a yellow wall with two framed sketches",
    "a squirrell driving a toy car",
    "a penguin standing on a sidewalk",
    "a basketball hoop with a large blue ball stuck in it",
    "a racoon holding a shiny red apple over its head",
    "a pair of glasses under a computer monitor",
    "a throw rug on a stone floor",
    "a beach with apartment buildings next to it",
    "a moose standing over a fox",
    "a massive statue of Hanuman",
    "a black t-shirt",
    "a small airplane",
    "an airplane taking off of a runway",
    "a monarch butterfly",
    "a lizard that just lost its tail",
    "a golden doorknocker on a red door",
    "a pitcher of orange juice",
    "a glass of orange juice",
    "a green pepper cut in half on a plate",
    "a green pepper sliced into many pieces",
    "a full pint of IPA",
    "a half-full pitcher of stout",
    "Times Square during the day",
    "Times Square at night",
    "the Kremlin at night",
    "a hot air balloon floating by the Kremlin",
    "a black shoe with a lightning bolt on it",
    "a plant with small flowers with purple petals",
    "a plant with orange flowers shaped like stars",
    "a flower with large yellow petals",
    "a yellow diamond-shaped sign",
    "an elephant in a tennis court",
    "a silver fire hydrant next to a sidewalk",
    "a yellow fire hydrant in grass",
    "a red fire hydrant by a brick wall",
    "a rusty fire hydrant surrounded by dirt",
    "a fire hydrant with graffiti on it",
    "a snake curled around a wooden post",
    "a grand piano with a white bench",
    "a piano with Christmas lights all over it",
    "a piano with an open song book above the keys ",
    "an ornate gold harp",
    "a massive modern windmill",
    "a red sport bike",
    "a painting of black and white vertical stripes",
    "a painting of black and white checkerboard",
    "a painting of black and white with a red border",
    "a team playing baseball",
    "children on a couch",
    "a woman with long hair",
    "a man standing under a tree",
    "a smiling man",
    "an elderly woman",
    "an old man",
    "a man with puppet",
    "a boy going to school",
    "a girl going to a farm",
    "a family on a road trip",
    "a father and a son playing tennis",
    "a man heading a soccer ball",
    "a sad man with green hair",
    "a woman with sunglasses and red hair",
    "a man looking at a distant mountain",
    "a woman looking at a house",
    "a man riding a horse",
    "a man chasing a cat",
    "a man chasing a horse",
    "a pumpkin on a man's head",
    "an angry man",
    "a frustrated child",
    "a laughing woman",
    "a woman running on a trail",
    "a boy jumping off a wall",
    "a girl diving into a pool",
    "a man banging on a door",
    "a comic book supervillian",
    "Downtown Istanbul at sunrise. detailed ink wash.",
    "Downtown Seattle at sunrise. detailed ink wash.",
    "Downtown Beijing at sunrise. detailed ink wash.",
    "Downtown Rio de Janeiro at sunrise. detailed ink wash.",
    "Downtown LA at sunrise. detailed ink wash.",
    "Downtown Sydney at sunrise. detailed ink wash.",
    "Downtown Sanfrancisco at sunrise. detailed ink wash.",
    "Downtown Singapore at sunrise. detailed ink wash.",
    "Downtown NYC at sunrise. detailed ink wash.",
    "Downtown Austin at sunrise. detailed ink wash.",
    "Downtown Shanghai at sunrise. detailed ink wash.",
    "Downtown Saigon at sunrise. detailed ink wash.",
    "A section of the Great Wall in the mountains. detailed charcoal sketch.",
    "The Great Hypostyle Hall of Karnak. detailed charcoal sketch.",
    "A Mesoamerican pyramid surrounded by jungle. detailed charcoal sketch.",
    "Taj Mahal with its reflection. detailed charcoal sketch.",
    "A spaceship made of cardboard.",
    "A old phonograph made of cardboard.",
    "A castle made of cardboard.",
    "A photo of a dragonfly made of water.",
    "A photo of a panda made of water.",
    "A photo of a teddy bear made of water.",
    "A photo of a crocodile made of water.",
    "A photo of a lotus flower made of water. ",
    "A photo of a maple leaf made of water. ",
    "A photo of a palm tree made of water. ",
    "A photo of a four-leaf clover made of water. ",
    "an abstract oil painting in deep red and black with a thick patches of white",
    "light and happiness throughout and finding its way to every corner of the world, abstract oil painting",
    "trying to find my way in a big confusing world, abstract oil painting",
    "the door of knowing, a portal brightly opening the way through darkness. abstract anime landscape oil painting.",
    "A soft beam of light shines down on an armored granite wombat warrior statue holding a broad sword. The statue stands an ornate pedestal in the cella of a temple. wide-angle lens. anime oil painting.",
    "Oil painting generated by artificial intelligence",
    "A heavy metal tiger standing on a rooftop while singing and jamming on an electric guitar under a spotlight. anime illustration.",
    "A funny Rube Goldberg machine made out of metal",
    "A funny Rube Goldberg machine made out of paper",
    "A funny Rube Goldberg machine made out of wood",
    "Oil painting of a giant robot made of sushi, holding chopsticks.",
    "Portrait of a gecko wearing a train conductor’s hat and holding a flag that has a yin-yang symbol on it. Charcoal.",
    "Portrait of a gecko wearing a train conductor’s hat and holding a flag that has a yin-yang symbol on it. Child's crayon drawing.",
    "Portrait of a gecko wearing a train conductor’s hat and holding a flag that has a yin-yang symbol on it. Chinese ink.",
    "Portrait of a gecko wearing a train conductor’s hat and holding a flag that has a yin-yang symbol on it. Comic.",
    "Portrait of a gecko wearing a train conductor’s hat and holding a flag that has a yin-yang symbol on it. Marble statue.",
    "Portrait of a gecko wearing a train conductor’s hat and holding a flag that has a yin-yang symbol on it. Oil on canvas.",
    "Portrait of a gecko wearing a train conductor’s hat and holding a flag that has a yin-yang symbol on it. Photograph.",
    "Portrait of a gecko wearing a train conductor’s hat and holding a flag that has a yin-yang symbol on it. Watercolor.",
    "Portrait of a gecko wearing a train conductor’s hat and holding a flag that has a yin-yang symbol on it. Woodcut.",
    "A rusty spaceship blasts off in the foreground. A city with tall skyscrapers is in the distance, with a mountain and ocean in the background. A dark moon is in the sky. realistic high-contrast anime illustration.",
    "A photo of an Athenian vase with a painting of pandas playing basketball in the style of Egyptian hieroglyphics",
    "A photo of an Athenian vase with a painting of pandas playing soccer in the style of Egyptian hieroglyphics",
    "A photo of an Athenian vase with a painting of pandas playing tennis in the style of Egyptian hieroglyphics",
    "A photo of an Athenian vase with a painting of pangolins playing basketball in the style of Egyptian hieroglyphics",
    "A photo of an Athenian vase with a painting of pangolins playing soccer in the style of Egyptian hieroglyphics",
    "A photo of an Athenian vase with a painting of pangolins playing tennis in the style of Egyptian hieroglyphics",
    "A photo of an Athenian vase with a painting of toucans playing basketball in the style of Egyptian hieroglyphics",
    "A photo of an Athenian vase with a painting of toucans playing soccer in the style of Egyptian hieroglyphics",
    "A photo of an Athenian vase with a painting of toucans playing tennis in the style of Egyptian hieroglyphics",
    "A shiny VW van in front of a cityscape. A smiling sloth stands on grass in front of the van and is wearing a leather jacket, a cowboy hat, a kilt and a bowtie. The sloth is holding a quarterstaff and a big book. High-contrast oil painting.",
    "A shiny VW van that has flowers painted on it. A smiling sloth stands on grass in front of the van and is wearing a leather jacket, a cowboy hat, a kilt and a bowtie. The sloth is holding a quarterstaff and a big book. ink sketch.",
    "Wood engraving of the Greek letter Omega",
    "a satellite image of a costal french city there is a large park on the west side and a mountain to the north. There is a cloud covering part of the image",
    "A gundam stands tall with its sword raised. A city with tall skyscrapers is in the distance, with a mountain and ocean in the background. A dark moon is in the sky. realistic high-contrast anime illustration.",
    "Hokusai's version of Animal Farm",
    "A photograph in a temple of a wall painting depicting pandas playing tennis, in the style of Egyptian tomb hieroglyphics",
    "an old raccoon wearing a top hat and holding an apple, oil painting in the style of van gogh",
    "portrait of a well-dressed raccoon, oil painting in the style of Rembrandt",
    "close-up portrait of a smiling businesswoman holding a cell phone, oil painting in the style of Rembrandt",
    "a portrait of a man wearing sunglasses and a business suit, painting in pop art style",
    "a sport car melting into a clock, surrealist painting in the style of Salvador Dali",
    "a stained glass window depicting a calm tyrannosaurus rex",
    "a tennis match in the style of Egyptian hieroglyphic tomb paintings",
    "vibrant portrait painting of Salvador Dalí with a robotic half face",
    "an espresso machine that makes coffee from human souls, high-contrast painting",
    "panda mad scientist mixing sparkling chemicals, high-contrast painting",
    "a dolphin in an astronaut suit on saturn, high-contrast painting",
    "The Oriental Pearl in sketch style",
    "The Oriental Pearl in Chinese painting",
    "The Oriental Pearl in oil painting",
    "a capybara made of voxels sitting in a field",
    "an emoji of a baby penguin wearing a blue hat, red gloves, green shirt, and yellow pants",
    "a professional high quality emoji of a lovestruck cup of boba",
    "a dutch baroque painting of a horse in a field of flowers",
    "Icon of a red heart",
    "A cartoon house with red roof",
    "A cartoon tiger face",
    "Face of an orange frog in cartoon style",
    "A vector illustration of a tree",
    "an abstract painting of the Empire State Building",
    "an abstract painting of the Sydney Opera House",
    "a painting of a white country home with a wrap-around porch",
    "an oil painting of a cat playing checkers",
    "a surrealist dream-like oil painting by salvador dalí of a cat playing checkers",
    "an oil painting of a hamster dragon",
    "a high-quality oil painting of a psychedelic hamster dragon",
    "a painting of a fox in the style of starry night",
    "a stained glass window of a panda eating bamboo",
    "a crayon drawing of a space elevator",
    "a futuristic city in synthwave style",
    "a pixel art corgi pizza",
    "a green clock in the shape of a pentagon",
    "a painting of a phone from the 20s",
    "a painting of the food of china",
    "a photograph of a bust of homer",
    "a drawing of a bust of homer",
    "an armchair in the shape of an avocado",
    "a painting of the skyline of New York City",
    "a drawing of the skyline of New York City",
    "a cartoon of a train going to the moon",
    "a cartoon of a cow jumping over the moon",
    "orange jello in the shape of a man",
    "a comic about a friendly car in the city",
    "a painting of street in Paris",
    "a painting of a canal in Venice",
    "an abstract painting with blue, red and black",
    "an abstract painting of a waterfall",
    "an abstract painting of a spaceship",
    "an abstract painting of a pond with a bridge",
    "a cartoon of an angry shark",
    "a cartoon of a happy car on the road",
    "a cartoon of a bear birthday party",
    "a drawing of a house on a mountain",
    "a cartoon of a house on a mountain",
    "a painting of a house on a mountain",
    "an abstract painting of a house on a mountain",
    "a painting of a peaceful lakeside landscape",
    "a drawing of a peaceful lakeside landscape",
    "a map of Australia",
    "a map of Italy",
    "a map of South America",
    "a map of Texas",
    "a map of Manhattan",
    "an impressionistic painting of tree and a building",
    "an abstract painting of a tree and a building",
    "a watercolor painting of a tree and a building",
    "an oil painting of a tree and a building",
    "an abstract painting of three triangles in blue, red and white",
    "an abstract painting of three triangles in blue, yellow and red",
    "an abstract painting of three squares in blue, red and white",
    "an abstract painting of three squares in blue, yellow and red",
    "a painting of a cute owl on a box",
    "a horse in a field in Minecraft style",
    "a drawing of a pig face with an eye patch",
    "a drawing of a hammer",
    "a drawing of a screwdriver",
    "a drawing of a handsaw",
    "a drawing of a power drill",
    "a cloud in the shape of a teacup",
    "a cloud in the shape of a elephant",
    "a cloud in the shape of a castle",
    "a footprint shaped like a peanut",
    "a drawing of a stork playing a violin",
    "The Statue of Liberty in Minecraft",
    "An oil painting of the Statue of Liberty",
    "An abstract painting of the Statue of Liberty",
    "a diagram of brain function",
    "a diagram of the human digestive system",
    "a diagram of a suburban house",
    "a stop sign with a blue background",
    "a painting of a sport car in the style of Monet",
    "a painting of a sport car in the style of Dali",
    "a drawing of a space shuttle in the style of da Vinci",
    "a coloring book page of a horse next to a stream",
    "a sketch of a horse",
    "a sketch of a skyscraper",
    "a sketch of a train",
    "a sketch of a camel next to a stream",
    "a diagram of the star constellations",
    "a thumbnail image of an ice cream cone",
    "a thumbnail image of a horse and cart",
    "background pattern with alternating roses and skulls",
    "graffiti of a funny dog on a street wall",
    "graffiti of a rocket ship on a brick wall",
    "a watercolor painting of a snowy owl standing in a grassy field",
    "a cute illustration of a horned owl with a graduation cap and diploma",
    "a cardboard spaceship",
    "the flag of the United Kingdom painted in rusty corrugated iron",
    "a diagram of the solar system",
    "a drawing of a pint of beer on a brick wall",
    "an abstract painting of the lights at Times Square",
    "an abstract drawing of the Great Wall",
    "a diagram of the Great Wall",
    "a map showing the Great Wall",
    "a still life painting of a pair of shoes",
    "an impressionist painting of the geyser Old Faithful",
    "a diagram of how the geyser Old Faithful works",
    "an abstract painting of the Great Pyramid",
    "a diagram of the inside of the Great Pyramid",
    "the silhouette of the Milllenium Wheel at dusk",
    "a painting of ten children on a couch",
    "a painting of a man standing on a street corner",
    "a painting of a man standing under a tree",
    "a drawing of a man standing under a tree",
    "a cartoon of a man standing under a tree",
    "a comic about a boy going to school",
    "a comic about a girl going to a farm",
    "a comic about a boy and a tiger",
    "a comic about a family on a road trip",
    "a comic about a father and a son playing tennis",
    "a cartoon of a boy playing with a tiger",
    "the mona lisa",
    "a painting of the Mona Lisa with a frown",
    "a painting of the Mona Lisa with New York City behind her",
    "the Mona Lisa in the style of Minecraft",
    "a thumbnail image of a person skiing",
    "a thumbnail image of a gingerbread man",
    "the cover of The Beatle's album Revolver",
    'the saying "BE EXCELLENT TO EACH OTHER" on a rough wall with a graffiti image of a green alien wearing a tuxedo.',
    'A green sign that says "Very Deep Learning" and is at the edge of the Grand Canyon.',
    "Portrait of a tiger wearing a train conductor's hat and holding a skateboard that has a yin-yang symbol on it. charcoal sketch",
    "Portrait of a tiger wearing a train conductor's hat and holding a skateboard that has a yin-yang symbol on it. child's crayon drawing",
    "Portrait of a tiger wearing a train conductor's hat and holding a skateboard that has a yin-yang symbol on it. Chinese ink and wash painting",
    "Portrait of a tiger wearing a train conductor's hat and holding a skateboard that has a yin-yang symbol on it. color ink-and-wash drawing",
    "Portrait of a tiger wearing a train conductor's hat and holding a skateboard that has a yin-yang symbol on it. comic book illustration",
    "Portrait of a tiger wearing a train conductor's hat and holding a skateboard that has a yin-yang symbol on it. marble statue",
    "Portrait of a tiger wearing a train conductor's hat and holding a skateboard that has a yin-yang symbol on it. oil painting",
    "Portrait of a tiger wearing a train conductor's hat and holding a skateboard that has a yin-yang symbol on it. photograph",
    "Portrait of a tiger wearing a train conductor's hat and holding a skateboard that has a yin-yang symbol on it. woodcut",
    "Two cups of coffee, one with latte art of a map of United States. The other has latte art of a map of Africa.",
    "Two cups of coffee, one with latte art of a lovely princess. The other has latte art of a frog.",
    "Two cups of coffee, one with latte art of a heart. The other has latte art of stars.",
    "Two cups of coffee, one with latte art of the Eiffel tower. The other has latte art of the Statue of Liberty.",
    "A sign that says Deep Learning",
    'A bar of chocolate without a wrapper that has the word "WRAPPER" printed on it.',
    'A glass of red wine tipped over on a couch, with a stain that writes "OOPS" on the couch.',
    "Two cups of coffee, one with latte art of yin yang symbol. The other has latte art of a heart.",
    '"G I G G L E" painted in thick colorful lettering as graffiti on a faded red brick wall with a splotch of exploding white paint.',
    'high-contrast image of the word "WOMBAT" written with thick colored graffiti letters on a white wall with dramatic splashes of paint',
    "A plush monkey fording the Charles River on a log while wearing a Boston Red Sox hat with MIT in the background.",
    "the letters P A X forming a very simple outline of an elephant's shape.  the elephant is facing left. vector art, orange logo.",
    'A burger patty, with the bottom bun and lettuce and tomatoes. "COFFEE" written on it in mustard',
    'Two cups of coffee, one with latte art of the words "LOVE" written in one. The other has latte art of the words "PEACE" written in the other.',
    'The saying "BE EXCELLENT TO EACH OTHER" written in a stained glass window.',
    'The saying "BE EXCELLENT TO EACH OTHER" written in faded paint on the hull of an old wooden boat and reflected in the water. Wide-angle lens.',
    'The saying "BE EXCELLENT TO EACH OTHER" written on a red brick wall with a graffiti image of a green alien wearing a tuxedo. A yellow fire hydrant is on a sidewalk in the foreground.',
    'The saying "BE EXCELLENT TO EACH OTHER" written with carved letters on driftwood.',
    "a hot air balloon with chameleon logo. the sun is shining and puffy white clouds are in the background.",
    "A portrait photo of a kangaroo wearing an orange hoodie and blue sunglasses standing on the grassin front of the Sydney Opera House holding a sign on the chest that says Welcome Friends!",
    'Anime illustration of a kangaroo holding a sign that says "Starry Night", in front of the Sydney Opera House sitting next to the Eiffel Tower under a blue night sky of roiling energy, exploding yellow stars, and radiating swirls of blu',
    'a group of cats in a meeting. there is a whiteboard with "stack more layers" written on it.',
    'A high contrast portrait photo of a fluffy hamster wearing an orange beanie and sunglasses holding a sign that says "Let\'s PAINT!"',
    'a cartoon of a dog saying "I see what you did there"',
    'a robot holding a sign with "Let\'s PAINT!" written on it',
    "a dog wearing a baseball cap backwards and writing BONEZ on a chalkboard",
    "graffiti spelling BE KIND on white subway tile",
    "a grumpy porcupine handing a check for $10,000 to a smiling peacock",
    "a boat with 'BLUE GROOVE' written on its hull",
    "a store front that has the word ‘openai’ written on it.",
    "a laptop with a maze sticker on it",
    "a yellow t-shirt with a dog on it",
    "a blue t-shirt with a dinosaur on it",
    "a bowl with a dinosaur on it",
    "a bowl with a cartoon dinosaur on it",
    "a flag with a dinosaur on it",
    "a flag with a drawing of a dinosaur on it",
    "a flag with three vertical stripes",
    "a flag with three vertical stripes that are blue, white and red",
    "a white towel with a cartoon of a cat on it",
    "a black towel with a cartoon of a dog on it",
    "a yellow wall with the word KA-BOOM on it",
    "a cat sitting on a box with a drawing of an elephant on it",
    "the word 'mardefly' on a coffee mug",
    "a stop sigh with 'ALL WAY' written below it",
    "a sticker stuck in the middle of a stop sign",
    "world's best brother t-shirt",
    "the cover of a book called 'Backpropaganda' by I.C. Gradients",
    "the saying 'do unto others as they would do unto you' written on a white background",
    "a t-shirt with Carpe Diem written on it",
    "a sweatshirt with 'Batch Norm' written on it",
    "a yellow sticky note with 'BUY MILK' written on it",
    "a large open book showing text and an illustration of a cat",
    "a yellow wall with 'Hello, world.' written on it",
    'a motorcycle parked in an ornate bank lobby with "BUZZ" written on its body',
    "a logo of a wombat on a coffee mug",
    "a coffee mug with an ankh symbol on it",
    "a t-shirt with 'ANKH' written on it",
    "a t-shirt with 'Archaelogy Rocks!' written on it",
    "a black t-shirt with the peace sign on it",
    "a store front with 'Grassy Meadow' written on it",
    "a store front with 'AwesomePurchase' written on it",
    "a laptop screen showing an internet search",
    "a laptop screen showing a bunch of photographs",
    "a laptop screen showing a document being edited",
    "a book with the words 'Don't Panic!' written on it",
    "the words 'KEEP OFF THE GRASS'",
    "the words 'KEEP OFF THE GRASS' written on a brick wall",
    "the words 'KEEP OFF THE GRASS' on a black sticker",
    "the words 'KEEP OFF THE GRASS' on a sign next to a lawn",
    "the word 'START'",
    "the word 'START' written on a street surface",
    "the word 'START' on a blue t-shirt",
    "the word 'START' written above the word 'SMILING'",
    "the word 'START' written in chalk on a sidewalk",
    "a wooden post with a yellow '3' painted on top",
    "a wooden post in front of a patch of tall grass",
    "a wooden post with a blue '5' painted on top",
    "a series of musical notes on a black t-shirt",
    "a series of musical notes on a computer screen"
]

window.PARTI_PROMPTS_SMALL = [
    "Four dragons surrounding a dinosaur",
    "a handsaw",
    "a stained glass window of a panda eating bamboo",
    "a horned owl with a graduation cap and diploma",
    "a present",
    "snow covering the Great Pyramid",
    "a piano",
    "the mona lisa wearing a cowboy hat and screaming a punk song into a microphone",
    "a Styracosaurus",
    "metal",
    "The frog found itself in the newspaper",
    "The buildings of downtown Manhattan situated at below Mount Everest. The Great Pyramid is in the foreground.",
    "A photo of a palm tree made of water. ",
    "a giraffe",
    "an F1",
    "a square with an angry face",
    "an abstract painting of the Sydney Opera House",
    "a painting of a cute owl on a box",
    "a map of Italy",
    "a spaceship that looks like the Sydney Opera House",
    "a wooden deck overlooking a mountain valley",
    "a watercolor painting of a snowy owl standing in a grassy field",
    "square blue apples on a tree with circular yellow leaves",
    "a surrealist dream-like oil painting by salvador dalí of a cat playing checkers",
    "bottles",
    "a white robot passing a soccer ball to a red robot",
    "several lily pads without frogs",
    "a red block to the left of a blue pyramid",
    "an avocado",
    "a capybara made of voxels sitting in a field",
    "an abstract oil painting in deep red and black with a thick patches of white",
    "a grand piano next to the net of a tennis court",
    "Fibonacci number",
    "Two cups of coffee, one with latte art of a heart. The other has latte art of stars.",
    "the word 'START'",
    "derision",
    "A shiny VW van with a cityscape painted on it and parked on grass.",
    "a chimpanzee",
    "a harp without any strings",
    "a store front",
    "a diagram of the star constellations",
    "Four deer surrounding a moose.",
    "a beach with a cruise ship passing by",
    "a tiger in a field",
    "A television made of water that displays an image of a cityscape at night.",
    "A tiger is playing football",
    "a cartoon of a house on a mountain",
    "a tennis court",
    "A green heart",
    "Mary gave the child the dog bit a Band-Aid.",
    "a drawing of the skyline of New York City",
    "a concert without any fans",
    "a cricketer standing next to a wicket",
    "a person with arms like a tree branch",
    "A helicopter flies over the Grand Canyon.",
    "a tornado",
    "a yellow sticky note",
    "a mountain stream with salmon leaping out of it",
    "a trash bin",
    "earth",
    "A bare kitchen has wood cabinets and white appliances",
    "Dogs playing poker",
    "Portrait of a tiger wearing a train conductor's hat and holding a skateboard that has a yin-yang symbol on it. oil painting",
    "A set of 2x2 emoji icons with happy, angry, surprised and sobbing faces. The emoji icons look like colorful macarons. All of the macarons are wearing cowboy hats.",
    "a steam locomotive speeding through a desert",
    "orange jello",
    "a bat landing on a baseball bat",
    "a volcano exploding next to a marina",
    "Three-quarters front view of a blue 1977 Ford F-150 coming around a curve in a mountain road and looking over a green valley on a cloudy day.",
    "The trophy doesn't fit into the brown suitcase because it's too large",
    "a dragon breathing fire onto a knight",
    "A photo of an Athenian vase with a painting of pangolins playing tennis in the style of Egyptian hieroglyphics",
    "A close-up high-contrast photo of Sydney Opera House sitting next to Eiffel tower, under a blue night sky of roiling energy, exploding yellow stars, and radiating swirls of blue",
    "two pianos next to each other",
    "An aerial view of Ha Long Bay without any boats",
    "a kitchen",
    "meaning of life",
    "a power drill on a table",
    "a baby daikon radish",
    "a comic about a family on a road trip",
    "The Statue of Liberty in Minecraft",
    "a yellow wall",
    "an abstract painting of three triangles in blue, yellow and red",
    "a car's wheel crushing a pair of headphones",
    "A photo of an Athenian vase with a painting of pandas playing tennis in the style of Egyptian hieroglyphics",
    "a sunken submarine at the bottom of the ocean",
    "Times Square",
    "a blue sports car on the road",
    "a red lego block",
    "view of a giraffe and a zebra in the middle of a field",
    "slices of avocado on a piece of toast",
    "a stack of three red cubes with a blue sphere on the right and two green cones on the left",
    "a pickup truck under street lights at night",
    "a man riding a camel on the beach",
    "A smiling sloth wearing a leather jacket, a cowboy hat, a kilt and a bowtie. The sloth is holding a quarterstaff and a big book. A shiny VW van with a cityscape painted on it and parked on grass.",
    "a Tyrannosaurus Rex roaring in front of a palm tree",
    "a car",
    "a white rabbit in blue jogging clothes doubled over in pain while a turtle wearing a red tank top dashes confidently through the finish line",
    "a corgi’s head depicted as an explosion of a nebula",
    "a painting of a white country home with a wrap-around porch",
    '"G I G G L E" painted in thick colorful lettering as graffiti on a faded red brick wall with a splotch of exploding white paint.',
    "a Tyrannosaurus Rex",
    "a photograph of sand with a bucket, lots of scattered shells but no sandpipers",
    "three violins lying on the floor",
    "a ballet dancer next to a waterfall",
    "a sword slicing through a loaf of bread",
    "a crayon drawing of a space elevator",
    "A bowl of Pho",
    "a cat chasing a horse",
    "a marina with a herd of dolphins playing in it",
    "a sign",
    "A black dog sitting on a wooden chair. A white cat with black ears is standing up with its paws on the chair.",
    "a long wooden bench in front of a brick wall",
    "a long-island ice tea cocktail",
    "wine bottles",
    "a chemtrail",
    "a three quarters view of a man getting into a car",
    "waterfall",
    "a watermelon chair",
    "a basketball to the left of two soccer balls on a gravel driveway",
    "a massive robot with a coffee cup head. it is standing in the street with one foot smashing a car.",
    "a comic about a boy going to school",
    "a flag furling in the wind",
    "a crown with a ruby in its center",
    "three black cats standing next to two orange cats",
    "a kids' book cover with an illustration of white dog driving a red pickup truck",
    "A Mesoamerican pyramid surrounded by jungle. detailed charcoal sketch.",
    "A helicopter flies over Yosemite.",
    "a painting of the mona lisa on a white wall",
    "a drawing of a stork playing a violin",
    "a giraffe imitating a turtle",
    "three yellow balls and two red boxes on a table",
    "a white flag with a red circle next to a solid blue flag",
    "a family of four posing in front of a house",
    "a tornado with a house carried along at its top",
    "a toy car in front of a teddy bear",
    "a tiny football in front of three yellow tennis balls",
    "a tree growing through a fence",
    "a long chemtrail trailing an airplane in a blue sky",
    "a rowboat",
    "A smiling sloth wearing a leather jacket, a cowboy hat and a kilt.",
    "a racoon detective using a microscope while riding in a train",
    "A spaceship made of cardboard.",
    "a cartoon of an angry shark",
    "brown white and black white guinea pigs eating parsley handed to them",
    "7 dogs sitting around a poker table",
    "four owls standing on a telephone wire",
    "a hat",
    "a laptop with a maze sticker on it",
    "a thumbnail image of a person skiing",
    "Portrait of a gecko wearing a train conductor’s hat and holding a flag that has a yin-yang symbol on it. Chinese ink.",
    "a flying boat gliding past the Parthenon",
    "an old-fashioned windmill surrounded by flowers",
    "beautiful fireworks in the sky with red, white and blue",
    "A tornado made of bees crashing into a skyscraper. painting in the style of Hokusai.",
    "a panda eating bamboo",
    "fireworks above the Parthenon",
    "a power drill",
    "A photo of a maple leaf made of water. ",
    "A raccoon wearing formal clothes, wearing a tophat and holding a cane. The raccoon is holding a garbage bag. Oil painting in the style of Vincent Van Gogh.",
    "a giraffe wearing a white bathing suit and carefully stepping to the edge of a diving board and preparing to dive",
    "a man eating a glazed donut and a woman eating a chocolate cake",
    "a cup of boba",
    "The Alamo",
    "a fog",
    "a young girl playing piano",
    "a black baseball hat with a flame decal on it",
    "A photograph of the inside of a subway train. There are frogs sitting on the seats. One of them is reading a newspaper. The window shows the river in the background.",
    "a chimpanzee wearing a bowtie and playing a piano",
    'The saying "BE EXCELLENT TO EACH OTHER" written in faded paint on the hull of an old wooden boat and reflected in the water. Wide-angle lens.',
    "orange jello in the shape of a man",
    "an owl",
    "A bowl of Beef Pho",
    "a circle",
    "a yellow wall with two framed sketches",
    "a collection of glasses",
    "A photograph of the inside of a subway train. There are lobsters sitting on the seats. One of them is reading a newspaper. The window shows the ocean in the background.",
    "graffiti spelling BE KIND on white subway tile",
    "A cozy living room with a painting of a corgi on the wall above a couch and a round coffee table in front of a couch and a vase of flowers on a coffee table",
    "a street in Paris",
    "a space elevator",
    "A photo of a panda made of water.",
    "a cat sitting on a stairway railing",
    "a chest",
    "a large open book",
    "a clock",
    "a child eating a birthday cake near some palm trees",
    "a zebra with blue stripes",
    "a pig face",
    "one child on a couch",
    "A portrait photo of a kangaroo wearing an orange hoodie and blue sunglasses standing on the grassin front of the Sydney Opera House holding a sign on the chest that says Welcome Friends!",
    "a street without vehicles",
    "a monarch butterfly hatching from its chrysalis",
    "a diagram of brain function",
    "The Oriental Pearl in Chinese painting",
    "a family of four walking at the beach with waves covering their feet",
    "an emoji of a baby penguin wearing a blue hat, red gloves, green shirt, and yellow pants",
    "A close-up of two mantis wearing karate uniforms and fighting, jumping over a waterfall.",
    "a child in the air while jumping on a trampoline",
    "a massive statue in a temple",
]

window.PARTI_PROMPTS_TINY = [
    "inertia",
    "the words 'KEEP OFF THE GRASS' written on a brick wall",
    "Two cups of coffee, one with latte art of a heart. The other has latte art of stars.",
    "a scientist",
    "a rusty fire hydrant surrounded by dirt",
    "a satellite image of a costal french city there is a large park on the west side and a mountain to the north. There is a cloud covering part of the image",
    "A photo of a panda made of water.",
    "a taxi driving in the countryside",
    "a coffee mug with an ankh symbol on it",
    "Portrait of a gecko wearing a train conductor’s hat and holding a flag that has a yin-yang symbol on it. Child's crayon drawing.",
]
